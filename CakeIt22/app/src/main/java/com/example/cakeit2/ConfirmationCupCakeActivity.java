...
public class ConfirmationCupCakeActivity extends AppCompatActivity {

    private DatabaseReference database;
    private FirebaseAuth auth;
    private TextView sponge, filling, frosting, toppings, candle, total, quantity;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_confirmation_cup_cake);
        ...
        database = FirebaseDatabase.getInstance().getReference("Custom Order").child(FirebaseAuth.getInstance().getCurrentUser().getUid());
                        ...
                        .addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                ...
                                if (user != null){
                                    ...
                                    String SPONGE, FILLING, FROSTING, TOPPINGS, CANDLE, QUANTITY, NAME, PRICE, ORDERID, ORDERSTATUS, USERNAME, USERCONTACT, UID, TIMEORDER, TIMERECEIVED;
                                    ...
                                    CustomCupCake_GlobalVariable customCupCake = new CustomCupCake_GlobalVariable(SPONGE,FILLING,FROSTING,TOPPINGS,CANDLE, QUANTITY, NAME, PRICE, ORDERID, ORDERSTATUS, USERNAME,USERCONTACT, UID, TIMEORDER, TIMERECEIVED);

                                    FirebaseDatabase.getInstance().getReference("Orders").child("Custom Orders").child("Cup Cake").child(ORDERID).setValue(customCupCake).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {

                                            // Storing in users database
                                            FirebaseDatabase.getInstance().getReference("User").child(FirebaseAuth.getInstance().getCurrentUser().getUid())
                                                    .child("My Orders").child("Custom Order").child("Cup Cake").child(ORDERID).setValue(customCupCake).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                @Override
                                                public void onComplete(@NonNull Task<Void> task) {
                                                    if (task.isSuccessful()) {
                                                        Toast.makeText(ConfirmationCupCakeActivity.this, "Your custom  Cupcake has been placed", Toast.LENGTH_SHORT).show();
                                                        Intent intent = new Intent(ConfirmationCupCakeActivity.this, HomeActivity.class);
                                                        startActivity(intent);
                                                        finish();
                                                    }else {
                                                        Toast.makeText(ConfirmationCupCakeActivity.this, "There is an error!", Toast.LENGTH_SHORT).show();
                                                    }
                                                }
                                            });
